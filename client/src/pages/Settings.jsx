  ...
  const handleValidateKey = useCallback(async (provider) => {
    ...
    setTimeout(() => {
      ...
      if (isValid) {
        ...
        toast.success(`La clé API pour ${provider} semble valide (à vérifier) !`);
      } else {
        setValidationStatus(prev => ({ ...prev, [provider]: { status: 'invalid', message: 'La clé semble trop courte.' } }));
        toast.error(`La clé API pour ${provider} semble invalide.`);
      }
    }, 1000);
  }, [settings]);
